...
- Works: seven 3:4 portrait examples form a single-row graphite gallery in file-number order; three images and a clear slice of the next card are visible on wide screens, the native scrollbar stays hidden, and previous/next arrows sit directly above the gallery. A separate unlabelled vertical VK video gallery follows below the photo track, separated by a hairline and whitespace
- Gallery controls: Works advances exactly one visible slot per arrow press and has no counter; Works and Interior retain native touch/trackpad scrolling and keyboard arrows; each image in both galleries opens a graphite full-screen lightbox without persistent overlay labels, with the uncropped image, arrows, Escape/outside-click close, and mobile swipe
