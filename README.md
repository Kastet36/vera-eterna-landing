...
`yclid`, сохранённые скриптом атрибуции. Цели Метрики `lead_visit` и `lead_price`
вызываются только после успешного ответа API. Отдельные цели с такими
идентификаторами нужно создать в интерфейсе Метрики для отчётов.
